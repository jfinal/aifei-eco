...
- 只读访问 Maven Central：`cn.aifei:aifei:1.1.1` 与 `redis.clients:jedis:7.5.2` 的 POM 均为 HTTP 200；本次检查 `cn.aifei:aifei-cache:1.1` POM 为 HTTP 404。该结果只代表查询时点，不证明命名空间发布权限或没有其他待发布任务。
