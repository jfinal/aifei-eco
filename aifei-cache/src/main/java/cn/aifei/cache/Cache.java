 ...
 * 本地缓存和分布式缓存共用接口。
 *
 * <p>缓存名称和键区分大小写且不能为空白，均可用冒号分级，缓存值不能为 {@code null}。
 * TTL 精度为毫秒，有效期不能小于一毫秒或大于 {@link Integer#MAX_VALUE} 秒（约 68 年）。</p>
 *
 * <p>调用方须避免不同名称与键的组合产生相同的 {@code cacheName + ":" + key}，
 * 并避免业务键前缀与可能执行 {@link #clear(String)} 的下级名称重叠。
 * 库不额外检查这些命名冲突，也不因键包含冒号而拒绝调用。
 * 冲突可能造成条目相互覆盖或被其他命名空间的清理操作删除，
 * 不在命名空间隔离与实现可替换保证范围内。</p>
 *
...
public interface Cache {
     ...
     *
     * <p>清理范围的隔离以调用方遵守本接口的命名责任为前提。
     * 例如在名称 {@code user} 下写入键 {@code profile:42} 后，
     * Redis 上的 {@code clear("user:profile")} 会清理该父级条目，
     * 即使目标子命名空间没有条目。此类前缀冲突由调用方避免并承担影响。</p>
     *
     * @param cacheName 缓存名称
     */
