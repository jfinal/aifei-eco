...
public class Cron4jPlugin implements Plugin {
	...
	private void addTask(Prop configProp, String configName) throws Exception {
		...
		if (StrUtil.isBlank(configNameValue)) {
		...
		for (String taskName : taskNameArray) {
			...
			if ( !(taskObj instanceof Runnable) && !(taskObj instanceof Task) ) {
				throw new IllegalArgumentException("Task 必须是 Runnable、CronTask、ProcessTask 或者 Task 类型");
			}
	...
	private static class TaskInfo {
		...
		void schedule() {
			if (enable) {
				...
				if (task instanceof Runnable) {
					...
					scheduler = null;
					throw new IllegalStateException("Task 必须是 Runnable、CronTask、ProcessTask 或者 Task 类型");
				}
		...
		void stop() {
			if (enable) {
				if (task instanceof CronTask) {   // 如果任务实现了 CronTask 接口，则回调 CronTask.stop() 方法
					((CronTask)task).stop();
				}
				scheduler.stop();
			}
