...
public class Cache {
    ...
    protected KeyNamingPolicy keyNamingPolicy;
    ...
    public Cache(String name, JedisPool jedisPool, Serializer serializer, KeyNamingPolicy keyNamingPolicy) {
        this.name = name;
    ...
    public KeyNamingPolicy getKeyNamingPolicy() {
    ...
    public List<Object> tx(String watchKeys, Consumer<Transaction> tx) {
        ...
        try {
            // 多个 watchKey 使用逗号分隔
            if (StrUtil.notBlank(watchKeys)) {
                String[] keys = watchKeys.split(",");
                for (String k : keys) {
                    if (StrUtil.notBlank(k)) {
                        jedis.watch(k.trim());
                        watched = true;
                    }
                }
    ...
    public List<Object> tx(Consumer<Transaction> tx) {
        return tx(null, tx);
    }
     ...
     */
    public void scan(Integer cursor, String pattern, Integer count, Function<List<String>, Boolean> keyList) {
        String cursorStr = cursor != null ? cursor.toString() : ScanParams.SCAN_POINTER_START;

        ScanParams scanParams = new ScanParams();
        if (StrUtil.notBlank(pattern)) {
    ...
    public void scan(Integer cursor, Function<List<String>, Boolean> keyList) {
